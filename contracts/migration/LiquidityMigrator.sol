...
contract LiquidityMigrator is BlackDaoAccessControlled {
    ...
    IERC20 internal immutable oldBLKD = IERC20(0x383518188C0C6d7730D91b2c03a03C837814a899);
    IERC20 internal immutable newBLKD = IERC20(0x64aa3364F17a4D01c6f1751Fd97C2BD3D7e7f1D5);
    IMigrator internal immutable migrator = IMigrator(0x184f3FAd8618a6F458C16bae63F70C426fE784B3);

    constructor(IBlackDaoAuthority _authority) BlackDaoAccessControlled(_authority) {}
    ...
    function migrateLP(
        ...
        uint256 initialNewBLKDBalance = newBLKD.balanceOf(address(this));
        ...
        (uint256 amountToken, uint256 amountBLKD) = routerFrom.removeLiquidity(
            token, 
            address(oldBLKD), 
            oldLPAmount,
            _minA, 
        ...
        // Migrate the V1 BLKD to V2 BLKD
        oldBLKD.approve(address(migrator), amountBLKD);
        migrator.migrate(amountBLKD, IMigrator.TYPE.UNSTAKED, IMigrator.TYPE.UNSTAKED);
        uint256 amountNewBLKD = newBLKD.balanceOf(address(this)).sub(initialNewBLKDBalance); // # V1 out != # V2 in

        // Add the V2 liquidity
        IERC20(token).approve(address(routerTo), amountToken);
        newBLKD.approve(address(routerTo), amountNewBLKD);
        ...
        newBLKD.safeTransfer(authority.governor(), newBLKD.balanceOf(address(this)));
        oldBLKD.safeTransfer(authority.governor(), oldBLKD.balanceOf(address(this)));
